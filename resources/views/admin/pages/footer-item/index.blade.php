...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <a href="{{ route('footer-item.create') }}" class="btn btn-primary">ایجاد عنوان</a>
                              ...
                              @foreach ($footerItem as $item)
                                 <tr>
                                    ...
                                    @switch($item->status)
                                       @case(0)
                                          @php
                                             $url = route('footer-item.status', $item->id);
                                    ...
                                    <td>{!! $status !!}</td>
                                    <td><a href="{{ route('footer-item.edit', $item->id) }}" class="btn btn-info">ویرایش</a></td>
                                    <td>
                                       <form action="{{ route('footer-item.destroy', $item->id) }}" method="POST">
                                          @csrf
